...
class StockPortfolioEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        tmp = [self.data[tech].values.tolist() for tech in self.tech_indicator_list ]
        # print('tmp',np.array(tmp).shape) # (8, 4)
        self.state =  np.append(np.array(self.covs), tmp, axis=0)
        # print('self.state',self.state,self.state.shape) #(12, 4)
        self.state = self.state.reshape(-1)
        # print('self.state',self.state,self.state.shape)#(48,)
...
#         print(actions)
        ...
        if self.terminal:
            ...
            return self.state, self.reward, self.terminal,(ann_reward,sharpe,self.asset_memory)
            ...
            self.reward = np.array([now_return,now_sharpe], dtype=np.float32) 
