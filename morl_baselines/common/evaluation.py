...
def eval_mo(
    ...
    while not done:
        if render:
            env.render()
        ans = env.step(agent.eval(obs, w))
        try:
            obs, r, terminated, truncated, info = ans
        except:
            # print(ans)
            obs, r, terminated, info = ans
...
def eval_mo_1time_info(
    ...
    while not done:
        ...
        # asset_memory_df.to_csv('asset_memory_df_'+mode+timestamp+'.csv')
