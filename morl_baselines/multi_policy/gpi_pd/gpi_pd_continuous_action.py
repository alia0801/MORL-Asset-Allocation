...
class Policy(nn.Module):
    ...
    def forward(self, obs, w, noise=None, noise_clip=None):
        ...
        # print('obs',obs,obs.shape) #(12,4)
        # print('w',w,w.shape) #(2)
        # print('obs.dim()',obs.dim()) #2
        h = self.latent_pi(th.cat((obs, w), dim=obs.dim() - 1))
...
class GPIPDContinuousAction(MOAgent, MOPolicy):
    ...
    def train_iteration(
        ...
        # print('tensor_w',tensor_w.shape)#2
        ...
        # print('obs',(th.tensor(obs).float()).shape) #obs torch.Size([12, 4])
        for _ in range(1, total_timesteps + 1):
            ...
            try:
                ...
                next_obs, vector_reward, terminated, info = self.env.step(action_env)

            # print('obs',obs,obs.shape)#(12, 4)
            # print('action',action,action.shape)#(4,)
            # print('vector_reward',vector_reward,vector_reward.shape)#(2,)
            # print('next_obs',next_obs,next_obs.shape)#(12, 4)
    ...
    def train(
        ...
        for iter in range(1, max_iter + 1):
            ...
            print("Next weight vector:", w)#[1. 0.]
            if weight_selection_algo == "gpi-ls":
